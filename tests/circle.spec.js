...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  ...
  it('Verifica se a função `circle` retorna um objeto com uma `key` com `value` igual a circunferência correta para um círculo de raio 2', () => {
    expect(circle(2)).toMatchObject({radius: 2});
  })
  it('Verifica se a função `circle` retorna um objeto com uma `key` com `value` igual a área correta para um círculo de raio 3', () => {
    expect(circle(3)).toMatchObject({area: 28.259999999999998});
  })
  it('Verifica se a função `circle` retorna um objeto comos dados corretos de um círculo de raio 3', () => {
