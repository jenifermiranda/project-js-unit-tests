...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  ...
  it('Verifica se quando não receber um parâmetro a função `getCharacter` retorna `undefined`', () => {
    expect(getCharacter()).toBeUndefined();
  });
  it('Verifica se a função `getCharacter` retorna o objeto correto para o parâmetro `Arya`', () => {
    expect(getCharacter('Arya')).toMatchObject({"class": "Rogue", "name": "Arya Stark", "phrases": ["Not today", "A girl has no name."]});
  });
  it('Verifica se a função `getCharacter` retorna o objeto correto para o parâmetro `Brienne`', () => {
    expect(getCharacter('Brienne')).toMatchObject({"class": "Knight", "name": "Brienne Tarth", "phrases": ["Im No Lady, Your Grace.", "I, Brienne Of Tarth, Sentence You To Die."]});
  });
  it('Verifica se a função `getCharacter` retorna o objeto correto para o parâmetro `Melissandre`', () => {
    expect(getCharacter('Melissandre')).toMatchObject({"class": "Necromancer", "name": "Melissandre", "phrases": ["Death By Fire Is The Purest Death.", "For The Night Is Dark And Full Of Terrors."]});
  });
  it('Verifica se o parâmetro não é Case Sensitive', () => {
    expect(getCharacter('arYa')).toStrictEqual({"class": "Rogue", "name": "Arya Stark", "phrases": ["Not today", "A girl has no name."]});
  });
  it('Verifica se aopassar um nome que não está na tabela, a função retorna `undefined`', () => {
